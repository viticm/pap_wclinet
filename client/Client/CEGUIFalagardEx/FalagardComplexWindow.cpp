...
//		d_ParentWindow->requestRedraw();
	...
	void FalagardComplexWindow::AddChildElement_Option(String strText,String strExtra1,String strExtra2, String strText3, FontBase* font)
	{
		...
		case 2:
			...
			break;
		case 3:
			pNewChild->d_Button = static_cast<FalagardButton*>(WindowManager::getSingleton().createWindow((utf8*)"TLBB_QuestButton_3", getName() + szTempName));//
			break;
		case 4:
			pNewChild->d_Button = static_cast<FalagardButton*>(WindowManager::getSingleton().createWindow((utf8*)"TLBB_QuestButton_4", getName() + szTempName));//
			break;
		default:
			pNewChild->d_Button = static_cast<FalagardButton*>(WindowManager::getSingleton().createWindow((utf8*)"TLBB_QuestButton_1", getName() + szTempName));//
			break;
...
//		pNewChild->d_Button = static_cast<FalagardButton*>(WindowManager::getSingleton().createWindow((utf8*)"WoWLook/Button_1", getName() + szTempName));
	...
	void FalagardComplexWindow::AddChildElement_Item(String strIconImage, String strText,  bool bRadio, bool bReArrange, FontBase* font)
	{
		...
		pNewChild->d_Button = static_cast<FalagardRewardItem*>(WindowManager::getSingleton().createWindow((utf8*)"TLBB_RewardItem", getName() + szTempName));
...
//			_snprintf(szTempName, 32, "__reward__%02d", (int)d_ChildVector.size());
		...
		pNewChild->d_ReArrange = bReArrange;
		pNewChild->d_Radio = bRadio;
		pNewChild->d_ActionButton = static_cast<FalagardActionButton*>(WindowManager::getSingleton().createWindow((utf8*)"TLBB_ActionButton", getName() + szTempName));
...
*/
		...
		pNewChild->d_Name = (utf8*)szTempName;
		pNewChild->d_ReArrange = bReArrange;
		pNewChild->d_Button = static_cast<FalagardButton*>(WindowManager::getSingleton().createWindow((utf8*)"TLBB_ButtonCommon", getName() + szTempName));
		pNewChild->d_Button->setText(strText);
		pNewChild->d_Button->show();
...
//		if(bRadio) d_RadioChild.push_back(pNewChild);
...

	{
		...
		pNewChild->d_Money = static_cast<FalagardMoney*>(WindowManager::getSingleton().createWindow((utf8*)"TLBB_Money"));
