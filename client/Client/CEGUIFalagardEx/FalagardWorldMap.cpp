...
namespace CEGUI
{
	...
	void FalagardWorldMap::createNewCity( int nCityID, CITY_DATA* data )
	{
		...
		if( !WindowManager::getSingleton().isWindowPresent( szWindowName ) ) // 如果不存在就创建一个
			data->pCityNameWindow = WindowManager::getSingleton().createWindow( "TLBB_Look_StaticImage", szWindowName );
		else
		...
		if( !WindowManager::getSingleton().isWindowPresent( szWindowName ) ) // 如果不存在就创建一个
			data->pCityWindow = WindowManager::getSingleton().createWindow( "TLBB_StaticImageNULL", szWindowName );
		else
		...
		if( !WindowManager::getSingleton().isWindowPresent( szWindowName ) ) // 如果不存在就创建一个
			data->pFriendWindow = WindowManager::getSingleton().createWindow( "TLBB_StaticImageNULL", szWindowName );
		else
		...
		if( !WindowManager::getSingleton().isWindowPresent( szWindowName ) ) // 如果不存在就创建一个
			data->pMyselfWindow = WindowManager::getSingleton().createWindow( "TLBB_StaticImageNULL", szWindowName );
		else
		...
		for( int i = 0; i < nY; i ++ )
			for( int j = 0; j < nX; j ++ )
			{
				...
				if( pImage && pImageset )
				{
					Window* pWindow = NULL;
					if( !WindowManager::getSingleton().isWindowPresent(strBackgroundName) ) 
						pWindow = WindowManager::getSingleton().createWindow( "TLBB_StaticImageNULL", strBackgroundName );
					else
