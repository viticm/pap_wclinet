...
public:
	...
	*************************************************************************/
	...
	\return
		The id of quad in render queue.
	*/
	virtual	unsigned long	addQuad(const Rect& dest_rect, float z, const Texture* tex, const Rect& texture_rect, 
		const ColourRect& colours, QuadSplitMode quad_split_mode, UserSharps bMenualSharp) = 0;

	/*!
	\brief
		adjust a quad in the list to be rendered.

	\param id
		the quad id that has aready in render queue.
	...
	\return
		Nothing
	*/
	virtual	void	adjustQuad(unsigned long id, const Texture* tex, const Rect& texture_rect, const ColourRect& colours) = 0;
