...
private:
	...
	*/
	...
	\param textureDelay
		Just load image define(not include texture) only.
...
public:
	...
	*/
	...
	/*!
	\brief
		true when texture not loaded.(load texture delay)		
	*/
	...
	*/
	unsigned long	draw(const Rect& source_rect, const Rect& dest_rect, float z, const Rect& clip_rect,const ColourRect& colours, QuadSplitMode quad_split_mode, UserSharps bMenualSharp) const;
	...
	\return
		The id of render quad in render queue
	*/
	unsigned int	draw(const Rect& source_rect, const Rect& dest_rect, float z, const Rect& clip_rect, const colour& top_left_colour = 0xFFFFFFFF, const colour& top_right_colour = 0xFFFFFFFF,  const colour& bottom_left_colour = 0xFFFFFFFF, const colour& bottom_right_colour = 0xFFFFFFFF, QuadSplitMode quad_split_mode = TopLeftToBottomRight, UserSharps bMenualSharp = LookLikeSquare) const
	{
		return draw(source_rect, dest_rect, z, clip_rect, ColourRect(top_left_colour, top_right_colour, bottom_left_colour, bottom_right_colour), quad_split_mode, bMenualSharp);
	}
...
protected:
	...
	*************************************************************************/
	...
	// dynamic load fields
	bool	d_textureLoadDelay;		//!< true when texture not loaded.(load texture delay)
	bool	d_needFree;
	bool    d_useAutoFree;
