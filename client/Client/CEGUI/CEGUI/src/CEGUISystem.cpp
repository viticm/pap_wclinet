...
System::System(Renderer* renderer, ScriptModule* scriptModule, const utf8* configFile) :
	d_clickTrackerPimpl(new MouseClickTrackerImpl)
{
    constructor_impl(renderer, NULL, NULL, scriptModule, configFile, (const utf8*)"UISystem.log");
}
...
System::System(Renderer* renderer, ScriptModule* scriptModule, ResourceProvider* resourceProvider, const utf8* configFile) :
	d_clickTrackerPimpl(new MouseClickTrackerImpl)
{
    constructor_impl(renderer, resourceProvider, NULL, scriptModule, configFile, (const utf8*)"UISystem.log");
}
...
System::System(Renderer* renderer, XMLParser* xmlParser, ScriptModule* scriptModule, const utf8* configFile) :
        d_clickTrackerPimpl(new MouseClickTrackerImpl)
{
    constructor_impl(renderer, NULL, xmlParser, scriptModule, configFile, (const utf8*)"UISystem.log");
}
...
System::System(Renderer* renderer, ResourceProvider* resourceProvider, XMLParser* xmlParser, ScriptModule* scriptModule, const utf8* configFile) :
        d_clickTrackerPimpl(new MouseClickTrackerImpl)
{
    constructor_impl(renderer, resourceProvider, xmlParser, scriptModule, configFile, (const utf8*)"UISystem.log");
}
...
void System::renderGUI(void)
{
	...
	if (d_gui_redraw)
	{
		...
		if( AnimateManager::getSingletonPtr() )
			AnimateManager::getSingleton().clearRuntimeAnimateList();
	...
	{
		if( AnimateManager::getSingletonPtr() )
			AnimateManager::getSingleton().updateRuntimeAnimateList();
...
/*************************************************************************
	Set the default font to be used by the system
*************************************************************************/
void System::setDefaultFont(FontBase* font)
{
	d_defaultFont = font;
...
bool System::injectMouseMove(float delta_x, float delta_y)
{
	...
	if (dest_window != NULL)
	{
		// MouseHollow (Ensure the window with mouse process MouseLeave Event!).
		if (dest_window->getMouseHollow())
		{
			if (d_wndWithMouse != NULL && d_wndWithMouse != dest_window)
			{
		...
		else if (dest_window != d_wndWithMouse)
		{
		...
		{
			// weather the window ignore all mouse input.
			//if(dest_window->getMouseHollow()) 
			//{
			...
			//}
			if( HyperLinkManager::getSingletonPtr() )
				if(HyperLinkManager::getSingleton().isHyperLinkRange(dest_window,ma.position))
				{
...
bool System::injectMouseButtonUp(MouseButton button)
{
        ...
        (tkr.d_target_window == initial_dest_window))
    {
		...
		while ((!ma.handled) && (dest_window != NULL))
		{
			...
			if( HyperLinkManager::getSingletonPtr() )
			{
				//if(HyperLinkManager::getSingleton().doHyperLink(dest_window,ma.position))
				//{
				//	ma.handled = true;
				//}
				//else
				if(button == LeftButton)
				{
					if(HyperLinkManager::getSingleton().doHyperLinkByMouseLeft(dest_window,ma.position))
						ma.handled = true;
				}
				else if(button == RightButton)
				{
					if(HyperLinkManager::getSingleton().doHyperLinkByMouseRight(dest_window,ma.position))
						ma.handled = true;
				}
			}
	...
	if(initial_dest_window && initial_dest_window->testClassName((CEGUI::utf8*)"FalagardChatBoard"))
	{
		return false;
	}
...
Window*	System::getTargetWindow(const Point& pt, int nEventType ) const
{
	...
	if (d_activeSheet != NULL)
	{
		...
		if (dest_window == NULL)
		{
			dest_window = d_activeSheet->getChildAtPosition(pt, nEventType);
...
Window* System::getKeyboardTargetWindow(void) const
{
	if(Ime::GetActiveIMEEditBox()) 
		return Ime::GetActiveIMEEditBox();
