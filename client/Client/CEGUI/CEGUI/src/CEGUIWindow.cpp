	...
	d_userSharp(LookLikeSquare)
{
	...
	d_userData		= NULL;
	d_needsRedraw   = true;
	d_mouseHollow	= false;//鼠标点击透过，但是移动不透
	d_mouseMoveHollow = false; // 说表移动透过，点击也透过
...
*************************************************************************/
const FontBase* Window::getFont(bool useDefault) const
{
	if (d_font == NULL)
	{
		...
		if ((*child)->isVisible())
		{
			...
			else
			{
				...
				if( (*child)->isHit(position) )
				{
					...
					{
					case 0: // 左键按下
						if( (*child)->getMouseHollow() )// 如果为镂空状态
							continue;
						break;
					case 1:
						if( (*child)->getMouseHollow() )// 如果为镂空状态
							continue;
						if( (*child)->getMouseLButtonHollow() ) 
							continue;
						break;
					case 2:
						if( (*child)->getMouseHollow() )// 如果为镂空状态
							continue;
						if( (*child)->getMouseRButtonHollow() )  // 如果右键hollow
							continue;
						break;
					case 3:
...
*************************************************************************/
void Window::setFont(const FontBase* font)
{
	d_font = font;
    WindowEventArgs args(this);
	onFontChanged(args);
...
void Window::renderPrepare(void)
{
	...
	// cause any child windows prepare
	UINT child_count = getChildCount();
...
void Window::prepareBigchar(void)
{
	...
	{
		((FontBase*)getFont())->prepareString(d_text);
	}
}
...
void Window::drawSelf(float z)
{
    ...
    if (d_renderCache.hasCachedImagery())
    {
        ...
        if (clipper.getWidth())
        {
            ...
            d_renderCache.render(absPos, z, clipper, d_userSharp);
        }
    }
}
...
/*************************************************************************
	...
	This always returns a valid obect.
*************************************************************************/
...
/*************************************************************************
    Helper method to judge a mouse button hollow state.
*************************************************************************/
