...
*************************************************************************/
unsigned long Imageset::draw(const Rect& source_rect, const Rect& dest_rect, float z, const Rect& clip_rect,
							 const ColourRect& colours, QuadSplitMode quad_split_mode, UserSharps bMenualSharp) const
{
	...
	if (final_rect.getWidth() != 0 && d_texture )
	{
		...
		return d_texture->getRenderer()->addQuad(final_rect, z, d_texture, tex_rect, colours, quad_split_mode, bMenualSharp);
...
{
	if(AnimateManager::getSingletonPtr() && AnimateManager::getSingleton().isImagesetInAnimate(this->getName()))
	{
...
void Imageset::AutoFree()
{
	...
	if( d_textureFilename.empty() )
		return;
	if( d_needFree == false ) // 如果不需要释放
	{
		d_needFree = true;// 设置释放标志
		return;
	}
