...
public:
	...
	\param target
		Pointer to an Ogre::RenderTarget object.
	...
	*/
	OgreCEGUIRenderer(Ogre::RenderTarget* target, Ogre::uint8 queue_id = Ogre::RENDER_QUEUE_OVERLAY, bool post_queue = false, UINT max_quads = 0);
	...
	*/
	OgreCEGUIRenderer(Ogre::RenderTarget* target, Ogre::uint8 queue_id, bool post_queue, UINT max_quads, Ogre::SceneManager* scene_manager);
	...
	*/
	...
	virtual	unsigned long	addQuad(const Rect& dest_rect, float z, const Texture* tex, const Rect& texture_rect, 
		const ColourRect& colours, QuadSplitMode quad_split_mode, UserSharps bMenualSharp = LookLikeSquare);

	// adjust a quad in the list
	virtual	void	adjustQuad(unsigned long id, const Texture* tex, const Rect& texture_rect, const ColourRect& colours);
...
private:
	...
	************************************************************************/
	...
	static const size_t    VERTEX_PER_ROUND;						 //!< number of vertices per quad
	...
	*************************************************************************/
	...
	void	constructor_impl(Ogre::RenderTarget* target, Ogre::uint8 queue_id, bool post_queue, UINT max_quads);
	...
	*************************************************************************/
	...
	Ogre::RenderTarget*			d_render_target;	//!< Pointer to the render target for Ogre.
	Ogre::uint8					d_queue_id;			//!< ID of the queue that we are hooked into
