...
namespace SCRIPT_SANDBOX
{
	...
	INT CGameProduce_Login::GetAreaCount(LuaPlus::LuaState* state)
	{
		...
		{
			//iAreaCount = 1;//CGameProcedure::s_pProcLogIn->GetAreaCount();//LM修改
...
*/
		...
		if(CGameProcedure::s_pProcLogIn)
		{
			//strInfo = "天上人间";//CGameProcedure::s_pProcLogIn->GetAreaName(nAreaIndex);//LM修改
...
*/
		...
		if(CGameProcedure::s_pProcLogIn)
		{
			...
			if(pInfo)
			{
				strInfoName  = pInfo->szAreaName;//LM修改
...
*/
		...
		{
			// 得到服务器名字
			//state->PushString("月牙");//(pLoginServerInfo->szLoginServerName.c_str());//LM修改
