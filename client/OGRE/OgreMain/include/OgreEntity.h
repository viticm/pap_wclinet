...
namespace Ogre {
	...
	*/
	...
	protected:
		...
		*/
		Entity( const String& name, const MeshPtr& mesh, bool suppressSkeletalAnimation = false);
		...
		*/
        ...
        /** Various dirty counter to records the update state
        */
        struct UpdateState
        {
            /// Records the last counter in which animation state set was updated
            ulong mAnimationStateCounter;
            /// Records the last counter in which manual bones was updated
            ulong mManualBoneCounter;

            UpdateState(void)
                : mAnimationStateCounter(std::numeric_limits<ulong>::max())
                , mManualBoneCounter(std::numeric_limits<ulong>::max())
            {
            }
        };

        /** Detect updates required for a given state, the state will filled with current state.
        */
        bool isUpdatesRequired(UpdateState& state) const;

		/// Cached bone matrices, including any world transform
        Matrix4 *mBoneWorldMatrices;
		/// The update state of the animation (included all the updates required).
        UpdateState mAnimationUpdateState;

		/// Perform all the updates required for an animated entity
		void updateAnimation(void);

        /** Class to store skeleton animation update state, might shares with other entity instances.
        */
        struct SkeletonUpdateState : UpdateState
        {
            /// Cached bone matrices in skeleton local space
            Matrix4 *mBoneMatrices;
            ushort mNumBoneMatrices;
		    bool mBoneMatricesOutOfDate;

            SkeletonUpdateState(ushort numBoneMatrices);
            ~SkeletonUpdateState();
        };

		/// The update state of the skeleton (exclued vertex skinning).
        /// It's a pointer because it can be shared between different entities with
        /// a shared skeleton.
        SkeletonUpdateState *mSkeletonUpdateState;
		...
		*/
        ...
        /// Private method to update skeletal animation (exclued vertex skinning).
        void updateSkeletalAnimation(void);
		...
		*/
		...
		void buildSubEntityList(const MeshPtr& mesh, SubEntityList* sublist);
	...
	protected:
		...
		public:
			...
			const HardwareVertexBufferSharedPtr& getWBuffer(void) const { return mWBuffer; }
	...
	public:
        ...
        */
        ...
        {
            return mSkeletonUpdateState ? mSkeletonUpdateState->mBoneMatrices : 0;
        }
		/** Internal method for retrieving bone matrix information. */
		unsigned short _getNumBoneMatrices(void) const
        {
            return mSkeletonUpdateState ? mSkeletonUpdateState->mNumBoneMatrices : 0;
        }
		...
		*/
        ...
        /** Start the skeletal animation in case is stopped previous or the entity
            was created with suppressed skeletal animation.
        @remarks
            The animation state set of this entity also refreshed.
        */
        void startSkeletalAnimation(void);

        /** Stop the skeletal animation.
        @remarks
            This also stop sharing the SkeletonInstance with other entities as well.
            And note that when skeletal animation was stopped, the SkeletonInstance
            of this entity also destroyed and the animation state set will refreshed.
        */
        void stopSkeletalAnimation(void);
		...
		*/
        ...
        /** Advance method to perform the updates of skeleton for an skeletally
            animated entity.
        @remarks
		    You don't normally need to call this, but it's here incase you wish to
            manually update the skeleton of an Entity at a specific point in time,
            e.g. you want to query the bone transforms but didn't want to animation
            updated completely. Skeleton will not be updated more than once a frame
            no matter how many times you call this method.
        @note
            This is a light-weight method, it'll update bone transforms only and
            never perform any vertex skinning/morphing, incase you need to access
            to skinned/morphed vertex data, you need to call _updateAnimation
            instead.
        */
