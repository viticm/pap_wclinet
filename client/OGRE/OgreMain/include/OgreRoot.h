...
namespace Ogre
{
    ...
    */
    ...
    private:
        ...
        typedef std::map<String, size_t> GeneratedNameSeqNumberMap;
        GeneratedNameSeqNumberMap mGeneratedNameSeqNumberMap;

		/// Are we initialised yet?
		bool mIsInitialised;

		bool mInnerPlugined;
		bool mRenderPlugined_D3D9;
		bool mRenderPlugined_GL;
    ...
    public:
		...
		*/
		...
		void loadInnerPlugins();

		void loadRenderPlugins_D3D9();

		void loadRenderPlugins_GL();

		void shutdownInnerPlugins();

		void unloadInnerPlugins();

		void unloadRenderPlugins();

		/* Render Plugins for SceneEditor */
		void loadRenderPlugins();
		void startupRenderPlugins(const String& name);
		...
		*/
        ...
        @remarks
            This function generate an unique name use the given prefix string. Most engine
            object create functions requires a name to identify the object, but in some
            case the name of the object is insignificant to you (e.g. you always access to
            the object by pointer). This function allow you create an unique name conveniently.
        @param prefix The prefix string use to generate unique name. You can use same prefix to
            generate unique name every time, this function will append a numeric postfix string
            to guarantee unique.
        */
        String generateUniqueName(const String& prefix);
    };
} // Namespace Ogre
#endif
