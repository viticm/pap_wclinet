    ...
    */
    ...
    public:
        ...
        */
        ...
        typedef std::map<unsigned short, NodeAnimationTrack*
            , std::less<unsigned short>
            , OgreFastPoolAllocator<std::pair<const unsigned short, NodeAnimationTrack*> >
        > NodeTrackList;
        typedef ConstMapIterator<NodeTrackList> NodeTrackIterator;

		typedef std::map<unsigned short, NumericAnimationTrack*
            , std::less<unsigned short>
            , OgreFastPoolAllocator<std::pair<const unsigned short, NumericAnimationTrack*> >
        > NumericTrackList;
		typedef ConstMapIterator<NumericTrackList> NumericTrackIterator;

		typedef std::map<unsigned short, VertexAnimationTrack*
            , std::less<unsigned short>
            , OgreFastPoolAllocator<std::pair<const unsigned short, VertexAnimationTrack*> >
        > VertexTrackList;
        ...
        @param
            respectsCurrentModeOnly If true, means respects current interpolation
            mode settings only. By default Ogre optimise keyframes with supports all
            possible interpolation mode settings, it's important for keep optimised
            animation work as same as original animation. But this result in that
            can't optimise best suitable for specifically interpolation mode, which
            you are intented to used only. In this case you can setup the interpolation
            mode that intented to used for this animation (via setInterpolationMode,
            setRotationInterpolationMode, etc), and then pass true in this parameter,
            Ogre will optimise the animation for that interpolation mode specially.
        @param
            toleranceFactor Used to evaluate that a keyframe is eliminable. If the
            difference between interpolated keyframe and original keyframe less than
            this factor, the keyframe is eliminable. It's a factor relatived to
            keyframe values, e.g. 0.05f means 5% of original keyframe values.
		*/
		void optimise(bool respectsCurrentModeOnly = false, Real toleranceFactor = 0.05f);
    ...
    protected:
		...
		void optimiseNodeTracks(bool respectsCurrentModeOnly, Real toleranceFactor);
		void optimiseVertexTracks(bool respectsCurrentModeOnly, Real toleranceFactor);
