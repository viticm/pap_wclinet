...
namespace Ogre {
    ...
    //-----------------------------------------------------------------------
    void MovableObject::destroy(void)
    {
        if (mManager)
        {
            // Destroy via scene manager
            mManager->destroyMovableObject(this);
        }
        else if (mCreator)
        {
            // Destroy via movable object factory
            mCreator->destroyInstance(this);
        }
        else
        {
            OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS,
                "Don't know how to destroy " + getMovableType() + " object \"" + mName + "\".",
