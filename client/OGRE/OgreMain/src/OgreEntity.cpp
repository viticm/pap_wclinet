...
namespace Ogre {
    ...
    Entity::Entity ()
		: mAnimationState(NULL),
		  ...
		  mAnimationUpdateState(),
          mSkeletonUpdateState(NULL),
    ...
    //-----------------------------------------------------------------------
    Entity::Entity(const String& name, const MeshPtr& mesh, bool suppressSkeletalAnimation) :
		MovableObject(name),
        mMesh(mesh),
        mAnimationState(NULL),
		mSkelAnimVertexData(0),
		...
		mNormaliseNormals(false)
{
        ...
        if (!mMesh->getSkeleton().isNull())
        {
            ...
            if (!suppressSkeletalAnimation)
            {
			    ...
			    mSkeletonUpdateState = new SkeletonUpdateState(mSkeletonInstance->getNumBones());
            }
        }
        ...
        if (mesh->isLodManual())
        {
            ...
            for (i = 1; i < numLod; ++i)
            {
                ...
                Entity* lodEnt = new Entity(name + "Lod" + StringConverter::toString(i),
                    usage.manualMesh, suppressSkeletalAnimation);
        ...
        if (!mMesh->getSkeleton().isNull() || hasVertexAnimation())
        {
            // Prepare temp blend buffers, we need do that even if skeleton
            // disabled, since we must remove blending info, as some old
            // hardware never supports UBYTE4.
    ...
    //-----------------------------------------------------------------------
    bool Entity::isUpdatesRequired(UpdateState& state) const
    {
        bool dirty = false;

        ulong counter;
        if (state.mAnimationStateCounter != (counter = mAnimationState->getDirtyFrameNumber()))
        {
            state.mAnimationStateCounter = counter;
            dirty = true;
        }

        if (hasSkeleton())
        {
            if (state.mManualBoneCounter != (counter = getSkeleton()->getManualBonesDirtyCounter()))
            {
                state.mManualBoneCounter = counter;
                dirty = true;
            }
        }
    ...
    void Entity::updateAnimation(void)
    {
        ...
        bool animationDirty = isUpdatesRequired(mAnimationUpdateState);
				...
				if (softwareAnimation)
				{
					...
					if (mSkelAnimVertexData)
					{
                        ...
                        Mesh::prepareMatricesForVertexBlend(blendMatrices,
                            mSkeletonUpdateState->mBoneMatrices, mMesh->sharedBlendIndexToBoneIndexMap);
					...
					for (i = mSubEntityList.begin(); i != iend; ++i)
					{
						...
						if (se->isVisible() && se->mSkelAnimVertexData)
						{
                            ...
                            // Prepare blend matrices, TODO: Move out of here
                            Mesh::prepareMatricesForVertexBlend(blendMatrices,
                                mSkeletonUpdateState->mBoneMatrices, se->mSubMesh->blendIndexToBoneIndexMap);
							// Blend, taking source from either mesh data or morph data
							Mesh::softwareVertexBlend(
								(se->getSubMesh()->getVertexAnimationType() != VAT_NONE)?
            ...
            if (hwAnimation && _isSkeletonAnimated())
            {
                ...
                if (!mBoneWorldMatrices)
                {
                    mBoneWorldMatrices = static_cast<Matrix4*>(
                        AlignedMemory::allocate(sizeof(Matrix4) * mSkeletonUpdateState->mNumBoneMatrices));
                }

                OptimisedUtil::getImplementation()->concatenateAffineMatrices(
                    mLastParentXform,
                    mSkeletonUpdateState->mBoneMatrices,
                    mBoneWorldMatrices,
                    mSkeletonUpdateState->mNumBoneMatrices);
            }
        }
    }
	//-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    void Entity::updateSkeletalAnimation(void)
    {
        if (isUpdatesRequired(*mSkeletonUpdateState))
        {
    ...
    void Entity::cacheBoneMatrices(void)
    {
        ...
        if (mSkeletonUpdateState->mBoneMatricesOutOfDate)
        {
            mSkeletonUpdateState->mBoneMatricesOutOfDate = false;
            mSkeletonInstance->_getBoneMatrices(mSkeletonUpdateState->mBoneMatrices);
        }
    }
    //-----------------------------------------------------------------------
    ...
    //-----------------------------------------------------------------------
    void Entity::buildSubEntityList(const MeshPtr& mesh, SubEntityList* sublist)
    {
        // Create SubEntities
    ...
    void Entity::prepareTempBlendBuffers(void)
    {
        ...
        // Need to clone and remove blending info even if skeleton disabled.
        if (!mMesh->getSkeleton().isNull())
        {
            // Shared data
            if (mMesh->sharedVertexData)
    ...
    void Entity::reevaluateVertexProcessing(void)
    {
        ...
        if (mAnimationState)
        {
            --mAnimationUpdateState.mAnimationStateCounter;
        }
    }
    //-----------------------------------------------------------------------
        ...
        bool extrude, Real extrusionDistance, unsigned long flags)
    {
        ...
        if (!mPreparedForShadowVolumes)
        {
            ...
            // dirty update state to force update of animations if they exist
            --mAnimationUpdateState.mAnimationStateCounter;
            // re-prepare buffers
            prepareTempBlendBuffers();
        }
        ...
        for (si = mShadowRenderables.begin(); si != siend; ++si, ++egi)
        {
            ...
            const HardwareVertexBufferSharedPtr& esrPositionBuffer = esr->getPositionBuffer();
    ...
    void Entity::shareSkeletonInstanceWith(Entity* entity)
    {
        ...
        {
            if (!entity->mSkeletonInstance)
            {
                OGRE_EXCEPT(Exception::ERR_RT_ASSERTION_FAILED,
                    "The entity has no skeleton instance.",
                    "Entity::shareSkeletonWith");
            }
            ...
            mSkeletonUpdateState = entity->mSkeletonUpdateState;
    ...
    //-----------------------------------------------------------------------
    void Entity::startSkeletalAnimation(void)
    {
        if (mSkeletonInstance)
        {
            OGRE_EXCEPT(Exception::ERR_RT_ASSERTION_FAILED,
                "This entity already started skeletal animation.",
                "Entity::startSkeletalAnimation");
        }
        if (mMesh->getSkeleton().isNull())
        {
            OGRE_EXCEPT(Exception::ERR_RT_ASSERTION_FAILED,
                "The mesh no skeleton at all.",
                "Entity::startSkeletalAnimation");
        }
    ...
    //-----------------------------------------------------------------------
    void Entity::stopSkeletalAnimation(void)
    {
        if (mMesh->getSkeleton().isNull())
        {
            OGRE_EXCEPT(Exception::ERR_RT_ASSERTION_FAILED,
                "The mesh no skeleton at all.",
                "Entity::stopSkeletalAnimation");
        }
        if (!mSkeletonInstance)
        {
            OGRE_EXCEPT(Exception::ERR_RT_ASSERTION_FAILED,
                "This entity already stopped skeletal animation.",
                "Entity::stopSkeletalAnimation");
        }
        ...
        // Need to re-prepare temp blend buffers to bound to original
        // buffers of the mesh, and remove blending info as well.
        prepareTempBlendBuffers();
    }
    //-----------------------------------------------------------------------
	...
	Entity::VertexDataBindChoice Entity::chooseVertexDataForBinding(bool vertexAnim) const
	{
        ...
        else if (!mMesh->getSkeleton().isNull())
        {
            // No vertex animation and skeleton disabled, need
            // to bind to original without blending info. Simulate
            // by software skeletal for now, note that other part
            // of code already guarantee this simulation worked.
            return BIND_SOFTWARE_SKELETAL;
        }
		else
		{
	...
	//-----------------------------------------------------------------------
    Entity::SkeletonUpdateState::SkeletonUpdateState(ushort numBoneMatrices)
        : mBoneMatrices(static_cast<Matrix4*>(AlignedMemory::allocate(sizeof(Matrix4) * numBoneMatrices)))
        , mNumBoneMatrices(numBoneMatrices)
        , mBoneMatricesOutOfDate(true)
    {
    }
	//-----------------------------------------------------------------------
    Entity::SkeletonUpdateState::~SkeletonUpdateState()
    {
		...
		const NameValuePairList* params)
	{
		...
		return new Entity(name, pMesh, suppressSkeletalAnimation);
