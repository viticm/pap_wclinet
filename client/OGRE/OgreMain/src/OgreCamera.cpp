...
#if !defined(__undef__)
        // Get points on "front" and "back" near plane in world space
        // Note: I can't explain why is 3 for "back" point, but it just work!
        Vector3 frontPoint = inverseVP * Vector3(nx, ny, -1.f);
        Vector3 backPoint = inverseVP * Vector3(nx, ny, +3.f);

        // The origin is in the middle of "front" and "back" point
        Vector3 origin = (frontPoint + backPoint) * 0.5f;

        // The direction is from "back" to "front" point
        Vector3 direction = (frontPoint - backPoint);
        direction.normalise();

        static int x[4];
        if (origin.positionCloses(rayOrigin))
            ++x[0];
        else
            ++x[1];
        if (direction.directionEquals(rayDirection, Radian(1e-3f)))
            ++x[2];
        else
            ++x[3];
        rayOrigin = origin;
