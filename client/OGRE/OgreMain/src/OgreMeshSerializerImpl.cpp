...
namespace Ogre {
    ...
    void MeshSerializerImpl::readMesh(DataStreamPtr& stream, Mesh* pMesh)
    {
        ...
        // Enable automatically build edge lists for this version only if user requested,
        // expect them in the file or not at all
        pMesh->setAutoBuildEdgeLists(
            MeshManager::getSingleton().getEnableAutoBuildAllMeshesEdgeLists());
        ...
        EdgeData* edgeData)
    {
        ...
        uint32 numTriangles = tmp[0];
        uint32 numEdgeGroups = tmp[1];
        ...
        for (size_t t = 0; t < numTriangles; ++t)
        {
            ...
            // unsigned long vertIndex[3]
            // unsigned long sharedVertIndex[3]
            readInts(stream, tmp, 8);
            tri.indexSet = tmp[0];
            tri.vertexSet = tmp[1];
            tri.vertIndex[0] = tmp[2];
            tri.vertIndex[1] = tmp[3];
            tri.vertIndex[2] = tmp[4];
            tri.sharedVertIndex[0] = tmp[5];
            tri.sharedVertIndex[1] = tmp[6];
            tri.sharedVertIndex[2] = tmp[7];
            // float normal[4]
        ...
        for (uint32 eg = 0; eg < numEdgeGroups; ++eg)
        {
            ...
            // unsigned long triCount
            // unsigned long numEdges
            readInts(stream, tmp, 4);
            edgeGroup.vertexSet = tmp[0];
            edgeGroup.triStart = tmp[1];
            edgeGroup.triCount = tmp[2];
            uint32 numEdges = tmp[3];
            ...
            for (uint32 e = 0; e < numEdges; ++e)
            {
                ...
                // unsigned long  vertIndex[2]
                // unsigned long  sharedVertIndex[2]
                readInts(stream, tmp, 6);
                edge.triIndex[0] = tmp[0];
                edge.triIndex[1] = tmp[1];
                edge.vertIndex[0] = tmp[2];
                edge.vertIndex[1] = tmp[3];
                edge.sharedVertIndex[0] = tmp[4];
                edge.sharedVertIndex[1] = tmp[5];
                // bool degenerate
