...
namespace Ogre {
		...
		if (mBuffersNeedRecreating)
		{
			...
			mIndexData->indexBuffer =
				HardwareBufferManager::getSingleton().createIndexBuffer(
					...
					mChainCount * (mMaxElementsPerChain + 1) * 6, // max we can use
    ...
    //-----------------------------------------------------------------------
    void BillboardChain::setChainLoop(size_t chainIndex, bool loop)
    {
		if (chainIndex >= mChainCount)
		{
			OGRE_EXCEPT(Exception::ERR_ITEM_NOT_FOUND,
				"chainIndex out of bounds",
				"BillboardChain::setChainLoop");
		}
    ...
    //-----------------------------------------------------------------------
    bool BillboardChain::getChainLoop(size_t chainIndex) const
    {
		if (chainIndex >= mChainCount)
		{
			OGRE_EXCEPT(Exception::ERR_ITEM_NOT_FOUND,
				"chainIndex out of bounds",
				"BillboardChain::getChainLoop");
		}
		...
		if (mIndexContentDirty)
		{
				...
				segi != mChainSegmentList.end(); ++segi)
			{
				...
				if (seg.head != SEGMENT_EMPTY && seg.head != seg.tail)
				{
                    ...
                    if (seg.loop)
                    {
                        // Start from head, which will connect with tail
                        laste = seg.tail;
                        e = seg.head;
                    }
                    else
