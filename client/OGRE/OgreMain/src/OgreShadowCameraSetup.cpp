...
namespace Ogre 
{
		...
		const Viewport *vp, const Light *light, Camera *texCam) const
	{
		...
		if (light->getType() == Light::LT_DIRECTIONAL)
		{
			...
			pos = target + dir * (shadowDist + sm->getShadowDirectionalLightExtrusionDistance());
		...
		*/
        ...
        // Should disable custom view/projection explicitly in case it's
        // enabled somewhere else (e.g. PlaneOptimalShadowCameraSetup).
        if (texCam->isCustomViewMatrixEnabled())
            texCam->setCustomViewMatrix(false);
        if (texCam->isCustomProjectionMatrixEnabled())
            texCam->setCustomProjectionMatrix(false);
	}
