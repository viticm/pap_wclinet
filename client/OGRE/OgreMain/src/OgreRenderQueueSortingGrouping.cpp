...
namespace Ogre {
    ...
    //-----------------------------------------------------------------------
    bool QueuedRenderableCollection::DepthSortDescendingLess::operator()(
        const RenderablePass& a, const RenderablePass& b) const
    {
        ...
        else
        {
            ...
            if (adepth == bdepth)
            {
                // Same depth, sort by pass hash
                uint32 hasha = a.pass->getHash();
                uint32 hashb = b.pass->getHash();
                if (hasha == hashb)
                {
