...
namespace Ogre
{
    ...
    bool TerrainRenderable::intersectSegment( const Vector3 & start, const Vector3 & end, Vector3 * result )
    {
        ...
        const Vector3 &minimum = getBoundingBox().getMinimum();
        const Vector3 &maximum = getBoundingBox().getMaximum();

        //start with the next one...
        ray += dir;


        while ( ! ( ( ray.x < minimum.x ) ||
            ( ray.x > maximum.x ) ||
            ( ray.z < minimum.z ) ||
            ( ray.z > maximum.z ) ) )
        {
