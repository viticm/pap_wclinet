...
void OctreeIntersectionSceneQuery::execute(IntersectionSceneQueryListener* listener)
{
    ...
    typedef std::set<MovablePair> MovableSet;
		...
		while( it.hasMoreElements() )
		{
			...
			while( nit != list.end() )
			{
				...
				while( oit.hasMoreElements() )
				{
					...
					{
						if (!listener->queryResult(e, m)) return;
						...
						if (m->getMovableType() == "Entity")
						{
							...
							while(childIt.hasMoreElements())
							{
								...
								{
									if (!listener->queryResult(e, c)) return;
								}
							}
						}
					}
...
void OctreeAxisAlignedBoxSceneQuery::execute(SceneQueryListener* listener)
{
    ...
    //grab all moveables from the node that intersect...
    OctreeSceneNodeList::iterator it = list.begin();
    while( it != list.end() )
    {
        ...
        while( oit.hasMoreElements() )
        {
            ...
            {
                if (!listener->queryResult(m)) return;
				// deal with attached objects, since they are not directly attached to nodes
				if (m->getMovableType() == "Entity")
				{
					...
					while(childIt.hasMoreElements())
					{
						...
						if (c->getQueryFlags() & mQueryMask)
						{
							if (!listener->queryResult(c)) return;
						}
					}
				}
            }
    ...
    while( it != list.end() )
    {
        ...
        while( oit.hasMoreElements() )
        {
				...
				(m->getTypeFlags() & mQueryTypeMask) && m->isInScene() )
            {
                ...
                if( result.first )
                {
                    if (!listener->queryResult(m, result.second)) return;
					// deal with attached objects, since they are not directly attached to nodes
					if (m->getMovableType() == "Entity")
					{
						Entity* e = static_cast<Entity*>(m);
						...
						while(childIt.hasMoreElements())
						{
							...
							if (c->getQueryFlags() & mQueryMask)
							{
								...
								if (result.first)
								{
									if (!listener->queryResult(c, result.second)) return;
								}
							}
						}
					}
...
void OctreePlaneBoundedVolumeListSceneQuery::execute(SceneQueryListener* listener)
{
    ...
    for (pi = mVolumes.begin(); pi != piend; ++pi)
    {
        ...
        //grab all moveables from the node that intersect...
        OctreeSceneNodeList::iterator it, itend;
        itend = list.end();
