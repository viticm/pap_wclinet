...
namespace Ogre {
    ...
    // Linear or arbvp1 fogged morphing terrain
	    ...
	    )
    {
	    ...
	    // Note: arbvp1 need fog coordinate
	    fog = oPosition.z;
    }
	    ...
	    uniform float fogDensity)
    {
	    ...
	    // Fog 
	    // f = e ^ - (camz x density)
	    fog = exp(- oPosition.z * fogDensity);
    }
	    ...
	    uniform float fogDensity)
    {
	    ...
	    colour = float4(1,1,1,1);
	    // Fog 
	    // f = e ^ - (camz x density)^2
	    float src1 = oPosition.z * fogDensity;
	    fog = exp(- src1*src1);
    }
