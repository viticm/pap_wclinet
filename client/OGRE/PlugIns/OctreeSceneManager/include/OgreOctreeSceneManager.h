...
typedef std::list < SceneNode *, OgreFastPoolAllocator<SceneNode*> > OctreeSceneNodeList;
...
public:
    ...
    */
    void findNodesIn( const AxisAlignedBox &box, OctreeSceneNodeList &list, SceneNode *exclude = 0 );

    /** Recurses the octree, adding any nodes intersecting with the sphere into the given list.
    It ignores the exclude scene node.
    */
    void findNodesIn( const Sphere &sphere, OctreeSceneNodeList &list, SceneNode *exclude = 0 );

    /** Recurses the octree, adding any nodes intersecting with the volume into the given list.
      It ignores the exclude scene node.
      */
    void findNodesIn( const PlaneBoundedVolume &volume, OctreeSceneNodeList &list, SceneNode *exclude=0 );

    /** Recurses the octree, adding any nodes intersecting with the ray into the given list.
      It ignores the exclude scene node.
      */
    void findNodesIn( const Ray &ray, OctreeSceneNodeList &list, SceneNode *exclude=0 );
